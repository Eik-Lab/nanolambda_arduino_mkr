...
namespace NanoLambdaNSP32
{
	...
	{
		public:
			ArduinoAdaptor(uint32_t gpioPinRst);		// constructor (for data channel == SPI)
			ArduinoAdaptor(uint32_t gpioPinRst, HardwareSerial *pSerial, NSP32::UartBaudRateEnum baudRate);	// constructor (for data channel == UART)

		private:
			uint32_t m_gpioPinRst;					// reset pin No. (GPIO pin to reset NSP32)
			HardwareSerial *m_pSoftSerial;			// pointer to SoftwareSerial
